...
from ProposedAlgorithms import count_month,GAWG,CUAM,level_membership_GAWG,level_membership_CUAM,print_com,transf,com_to_group,change,generate_patches,quality,col_map_level,transf_cases_gto,CDAD,dis_fact_half,mat_dis_norm_max
...
u_per,com_per = GAWG(W_jun_feb)
#Number of communities detected
n_com = len(com_per)
#Belonging level of each node to its respective community
belong_fact = level_membership_GAWG(u_per)
...
ax.legend(handles = color_patches,prop={'size':20})
...
for i in range(n_com-1,1,-1):
    com_i_per,mat_per_i,u_i_per = CUAM(u_per,com_per,W_jun_feb,i)
...
print(t)
print(red_mod)
plot(t,red_mod,linestyle="--",marker="o",markersize=6,linewidth=1.0,color='blue')
ylabel("Modularity")
xlabel("Number of communities")
axhline(y=0.46676348,color='red',linestyle="--")
title("Quality of partitions resulting from the decreasing of communities")
grid()
...
#Partition using CUAM, that considers sums of modularity between pairs of communities
com4_jun_feb,W_mod_jun_feb,u4_jun_feb = CUAM(u_per,com_per,W_jun_feb,4)
print("Division into 4 communities for the period June 2021-February 2022:")
print_com(W_jun_feb,com4_jun_feb,name_mun)
#Belonging level of each node to its respective community
belong4_jun_feb = level_membership_GAWG(u4_jun_feb)
print()
...
#Assign community labels to colors
color_patches = generate_patches(colors)
ax.legend(handles=color_patches,prop={'size':16})
...
"""#Belonging level to the communities

Color belonging level using geopandas
"""
...
titulo='Belonging level to community 0'
col = 'Niv0_4'
...
fig.savefig("belong_CUAM_com3.png",dpi=600)
...
for i in range(n_com+1,n_com+11):
    com_i_per = CDAD(com_per,i,W_jun_feb,dis_fact_half,mat_dis_norm_max)
    n_com_i = len(com_i_per)
    B,mod = quality(W_jun_feb,com_i_per)
    print(mod)
    aum_mod.append(mod)
...
print(t)
print(aum_mod)
plot(t,aum_mod,linestyle="--",marker="o",markersize=6,linewidth=1.0,color='blue')
ylabel("Modularity")
xlabel("Number of communities")
axhline(y=0.46676348,color='red',linestyle="--")
title("Quality of partitions resulting from the increasing of communities")
grid()
...
com10_per = CDAD(com_per,10,W_jun_feb,dis_fact_half,mat_dis_norm_max)
...
#Assign community labels to colors
color_patches = generate_patches(colors)
ax.legend(handles=color_patches,prop={'size':17})
